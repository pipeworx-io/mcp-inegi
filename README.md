...
`tools/list` at `https://gateway.pipeworx.io/inegi/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/inegi_population \
  -H 'Content-Type: application/json' \
  -d '{"area":"Mexico"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/inegi_population`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
