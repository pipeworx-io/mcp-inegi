...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'INEGI');
}
...
async function inegiFetch(
  ...
  const res = await pwFetch(url, { headers: { Accept: 'application/json', 'User-Agent': 'Pipeworx/1.0 (pipeworx.io)' } });
  if (!res.ok) throw await httpError(res, 'INEGI API error');
